    ...
    <form action={action} className="stack-form">
      ...
      <label>
        ...
        <input
          ...
          placeholder="Pine Ridge Explorers"
        />
      </label>
      <label>
        Welcome greeting
        <textarea
          ...
          maxLength={500}
          defaultValue={
            crew?.welcome_greeting ||
            "Welcome, explorers! Let’s learn and grow together."
          }
        />
      </label>
      <label>
        ...
        <input
          ...
          type="url"
          placeholder="https://…"
          defaultValue={crew?.logo_url || ""}
        />
      </label>
      <p>Each new player starts with $1,000 in pretend money.</p>
      <button className="primary-button" disabled={pending}>
        {pending ? "Saving…" : crew ? "Save crew" : "Create crew"}
      </button>
      {state.message && (
...
export function InvitationForm({ crews }: { crews: Crew[] }) {
  ...
  async function copyLink() {
    try {
      ...
      setCopied("Select the invitation link and copy it from the browser.");
    }
  }
  return (
    <form action={action} className="stack-form">
      <label>
        Crew
        <select name="crew" required defaultValue="">
          <option value="" disabled>
            Select a crew
          </option>
          {crews.map((c) => (
            <option key={c.id} value={c.id}>
              {c.name} · {c.public_code}
            </option>
          ))}
        </select>
      </label>
      ...
      <button className="primary-button" disabled={pending || !crews.length}>
      ...
      {state.invitePath && (
        <div className="invite-result">
          <a href={state.invitePath} target="_blank" rel="noreferrer">
            Open invitation
          </a>
    ...
    <div className="user-controls">
      <form action={roleAction}>
        ...
        <input
          type="hidden"
          name="role"
          value={profile.role === "admin" ? "player" : "admin"}
        />
        <button className="text-button" disabled={rolePending}>
      ...
      {profile.role === "player" && (
        <details>
          ...
          <form action={deleteAction} className="stack-form">
            <input type="hidden" name="id" value={profile.id} />
            <p>
            ...
            {deleteState.message && <p role="status">{deleteState.message}</p>}
          </form>
        </details>
      )}
    </div>
