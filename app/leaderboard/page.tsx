...
export default async function LeaderboardPage() {
  ...
  if (!crew)
    return (
      <AppShell profile={profile}>
        <PageIntro
          eyebrow="Your crew"
          title="A crew is waiting for you"
          text="Ask your crew leader to finish your membership before viewing the leaderboard."
        />
      </AppShell>
    );
  ...
  try {
    totals = new Map(
      (await leaderboard(crew.public_code)).map((p) => [
        p.player_id,
        p.total_value_cents,
      ]),
    );
  ...
  return (
    <AppShell profile={profile} active="leaders">
      <PageIntro
        eyebrow={crew.name}
      ...
      {unavailable ? (
        <p className="notice" role="status">
          Portfolio values are temporarily unavailable. Please check back soon.
        </p>
      ) : (
        <LeaderList
