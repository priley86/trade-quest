...
export async function saveCrew(
  ...
  try {
    const values = {
      name: requiredText(form.get("name"), "Crew name"),
      welcome_greeting: requiredText(
        form.get("welcome"),
        "Welcome greeting",
        500,
      ),
      logo_url: logoUrl(form.get("logo")),
    };
    ...
    const result = id
      ...
      : await supabase
          ...
          .insert({
            ...values,
            public_code: `CREW-${randomBytes(6).toString("hex").toUpperCase()}`,
            created_by: user.id,
          ...
          .single();
    if (result.error)
      return { message: "Couldn’t save the crew. Please try again." };
    ...
    return {
      ...
      message: id
        ? "Crew updated."
        : "Crew created! Make an invitation below to welcome your explorers.",
    };
  } catch (error) {
...
export async function createInvitation(
  ...
  try {
    ...
    if (error)
      return {
        message:
          "Couldn’t create the invitation. Select an existing crew and try again.",
      };
    revalidatePath("/admin");
    return {
      success: true,
      message:
        "Invitation ready. Copy this link now; it won’t be shown again after leaving this page.",
...
export async function changeRole(
  ...
  try {
    ...
    if (error)
      return {
        message:
          "Couldn’t change this role. Reload and check your admin access.",
      };
    revalidatePath("/admin");
...
export async function deleteUser(
  ...
  try {
    ...
    const { error: claimError } = await viewerClient.rpc(
      "claim_player_deletion",
      { target_user: target },
    );
    if (claimError)
      return {
        message:
          "Couldn’t delete this player. Admins must be demoted first, and accounts already being deleted cannot be selected.",
      };
