...
const headers = process.env.CARDSIGHT_API_KEY
  ? {
      Authorization: `Bearer ${process.env.CARDSIGHT_API_KEY}`,
...
const [holdings] = await db.query(
  "SELECT id,asset_public_id FROM holdings WHERE asset_type='sports_card'",
);
...
for (let start = 0; start < holdings.length; start += 10) {
  ...
  for (const h of batch) {
    try {
      const r = await fetch(
        `${base}/v1/pricing/${encodeURIComponent(h.asset_public_id)}`,
        { headers },
      );
      if (!r.ok) throw new Error(`API HTTP ${r.status}`);
      const j = await r.json();
      const records = (j?.raw?.records || [])
        .filter((x) => Number.isFinite(Number(x.price)))
        .sort((a, b) => String(b.date).localeCompare(String(a.date)))
        .slice(0, 10);
      if (!records.length) {
        console.warn(
          `[sports-values] SKIPPED ${h.id} (${h.asset_public_id}): no raw records`,
        );
        skipped++;
        continue;
      }
      const cents = Math.round(
        (records.reduce((s, x) => s + Number(x.price), 0) / records.length) *
          100,
      );
      ...
      await db.query(
        "INSERT INTO holding_price_history (holding_id,recorded_date,market_value_cents,source,card_api_id) VALUES (?,CURRENT_DATE,?,'cardsight-10-sales',?) ON DUPLICATE KEY UPDATE market_value_cents=VALUES(market_value_cents)",
        [h.id, cents, h.asset_public_id],
      );
      console.log(
        `[sports-values] UPDATED ${h.id} (${h.asset_public_id}) -> $${(cents / 100).toFixed(2)}`,
      );
      updated++;
    } catch (e) {
      console.error(
        `[sports-values] FAILED ${h.id} (${h.asset_public_id}): ${e.message}`,
      );
      failed++;
    }
...
for (const a of accounts) {
  const [v] = await db.query(
    "SELECT COALESCE(SUM(current_value_cents),0) total FROM holdings WHERE player_id=?",
    [a.player_id],
  );
...
console.log(
  `[sports-values] complete: updated=${updated} skipped=${skipped} failed=${failed}`,
);
await db.end();
