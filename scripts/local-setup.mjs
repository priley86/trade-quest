...
if (!api || !["127.0.0.1", "localhost"].includes(new URL(api).hostname))
  throw new Error("This setup script only supports local Supabase.");
const anon = status.ANON_KEY;
const service = status.SERVICE_ROLE_KEY;
if (!anon || !service)
  throw new Error(
    "Local Supabase keys are unavailable. Run npm run local:supabase first.",
  );
const env = [
  `NEXT_PUBLIC_SUPABASE_URL=${api}`,
  `NEXT_PUBLIC_SUPABASE_ANON_KEY=${anon}`,
  `SUPABASE_SERVICE_ROLE_KEY=${service}`,
  "DOLT_DATABASE_URL=mysql://root@127.0.0.1:3307/tradequest",
  "DOLTHUB_DATABASE=priley86/trade-quest",
  "DOLTHUB_BRANCH=main",
  "DOLTHUB_API_TOKEN=",
  "",
].join("\n");
try {
  ...
  if (
    existingUrl &&
    !["localhost", "127.0.0.1"].includes(new URL(existingUrl).hostname)
  )
    throw new Error("Refusing to overwrite hosted .env.local settings.");
} catch (error) {
...
await connection.query(
  "ALTER TABLE holdings ADD COLUMN IF NOT EXISTS product_url varchar(2048) NOT NULL DEFAULT ''",
);
await connection.query(
  "ALTER TABLE holdings ADD COLUMN IF NOT EXISTS image_url varchar(2048)",
);
const db = createClient(api, service, {
...
if (!crew) {
  const { data, error } = await db
    .from("crews")
    .insert({
      public_code: "PINE-742",
      name: "Pine Ridge Explorers",
      welcome_greeting:
        "Welcome, adventurers! Let’s learn, trade, and grow together.",
      created_by: user.id,
    })
    .select("*")
    .single();
...
for (const result of [
  ...
  await db.from("ledger_enrollments").upsert(
    {
      user_id: user.id,
      player_id: profile.public_player_id,
      crew_public_id: crew.public_code,
...
await connection.execute(
  "INSERT IGNORE INTO player_accounts (player_id,crew_public_id,display_name,cash_cents) VALUES (?,?,?,100000)",
  [profile.public_player_id, crew.public_code, profile.display_name],
);
...
console.log(
  "Local credentials and invitation are saved in .local/welcome.txt (ignored by Git).",
);
