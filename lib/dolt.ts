...
function localPool(): Pool | null {
  ...
  if (!url) return null;
  local.tradequestDolt ??= mysql.createPool({
    uri: url,
    connectionLimit: 5,
    connectTimeout: 5000,
  });
...
export async function buyPokemon(
  ...
  if (!pool)
    throw new Error(
      "Purchases are available with local Dolt first; hosted ledger writes are coming next.",
    );
  const id = sqlText(uuid(playerId));
  const price = Math.round(product.nearMintNormalCents);
  if (!Number.isSafeInteger(price) || price < 1)
    throw new Error("This card does not have a valid price.");
  const conn = await pool.getConnection();
  try {
    await conn.beginTransaction();
    const [accounts] = await conn.query<RowDataPacket[]>(
      `SELECT cash_cents FROM player_accounts WHERE player_id=${id} FOR UPDATE`,
    );
    if (!accounts[0])
      throw new Error("Open your portfolio before buying a card.");
    if (Number(accounts[0].cash_cents) < price)
      throw new Error("You don’t have enough pretend money for this card.");
    const text = (v: string) => sqlText(v);
    await conn.query(
      `UPDATE player_accounts SET cash_cents=cash_cents-${price} WHERE player_id=${id}`,
    );
    await conn.query(
...
export async function buySports(
  playerId: string,
  product: {
    id: string;
    name: string;
  ...
  const id = sqlText(uuid(playerId));
  const price = Math.round(product.marketPriceCents);
  const conn = await pool.getConnection();
  try {
    await conn.beginTransaction();
    ...
    if (!a[0] || Number(a[0].cash_cents) < price)
      throw new Error("You don’t have enough pretend money for this card.");
    const t = (v: string) => sqlText(v);
    await conn.query(
      `UPDATE player_accounts SET cash_cents=cash_cents-${price} WHERE player_id=${id}`,
    );
    await conn.query(
...
export async function sellHolding(playerId: string, holdingId: string) {
  ...
  if (!pool)
    throw new Error(
      "Selling is available with local Dolt first; hosted ledger writes are coming next.",
    );
  const id = sqlText(uuid(playerId));
  const hid = sqlText(holdingId);
  const conn = await pool.getConnection();
  try {
    await conn.beginTransaction();
...
function endpoint() {
  ...
  if (!database || !/^[A-Za-z0-9_-]+\/[A-Za-z0-9_.-]+$/.test(database))
    throw new Error("The game ledger is not connected yet.");
  return `https://www.dolthub.com/api/v1alpha1/${database}`;
}
function branch() {
  return encodeURIComponent(process.env.DOLTHUB_BRANCH || "main");
}
export async function readQuery<T>(query: string): Promise<T[]> {
  ...
  if (!response.ok)
    throw new Error("The game ledger is temporarily unavailable.");
  const result = await response.json();
  if (
    result.query_execution_status !== "Success" ||
    !Array.isArray(result.rows)
  ) {
    throw new Error(
      "The game ledger could not return a complete result. Ask your crew leader to check setup.",
    );
  }
  return result.rows;
}
export async function portfolio(playerId: string): Promise<Portfolio | null> {
  const id = sqlText(uuid(playerId));
  const accounts = await readQuery<Omit<Portfolio, "holdings" | "snapshots">>(
  ...
  return {
    ...accounts[0],
    cash_cents: Number(accounts[0].cash_cents),
    holdings: holdings.map((h) => ({
      ...h,
      quantity: Number(h.quantity),
      cost_basis_cents: Number(h.cost_basis_cents),
      current_value_cents: Number(h.current_value_cents),
    })),
    snapshots: snapshots
      .reverse()
...
export async function provisionPortfolio(enrollment: Enrollment) {
  ...
  if (!token)
    throw new Error("Your crew leader still needs to connect the game ledger.");
  const headers = {
  ...
  const response = await fetch(`${endpoint()}/write/${branch()}/${branch()}`, {
    method: "POST",
    headers,
    body: JSON.stringify({ query: enrollmentQuery(enrollment) }),
    cache: "no-store",
    signal: AbortSignal.timeout(10000),
  });
  if (!response.ok)
    throw new Error("Couldn’t open your portfolio yet. Please try again.");
  const result = await response.json();
  if (result.query_execution_status !== "Success" || !result.operation_name)
    throw new Error("Couldn’t open your portfolio yet. Please try again.");
  ...
  for (let attempt = 0; attempt < 8; attempt++) {
    ...
    if (operation.done) {
      if (operation.res_details?.query_execution_status !== "Success")
        throw new Error(
          "Portfolio setup needs another try. Your starting money will only be added once.",
        );
      return;
    }
  }
  throw new Error(
    "Your portfolio is still being prepared. Wait a moment, then refresh or try again. Your starting money will only be added once.",
  );
}
export async function buyStock(
  playerId: string,
  product: { symbol: string; price: number; shares: number },
) {
  const pool = localPool();
  if (!pool) throw new Error("Purchases are available with local Dolt first.");
  const id = sqlText(uuid(playerId));
  const shares = Number(product.shares);
  const price = Math.round(product.price * 100 * shares);
  if (
  ...
  try {
    ...
    if (!a[0] || Number(a[0].cash_cents) < price)
      throw new Error("You don’t have enough pretend money for this purchase.");
    const t = (v: string) => sqlText(v);
    await conn.query(
      `UPDATE player_accounts SET cash_cents=cash_cents-${price} WHERE player_id=${id}`,
    );
