...
for (const account of accounts) {
  const [dates] = await db.query(
    "SELECT DISTINCT recorded_date AS d FROM holding_price_history UNION SELECT DATE(executed_at) AS d FROM trades WHERE player_id=? UNION SELECT CURRENT_DATE AS d ORDER BY d",
    [account.player_id],
  );
  ...
  for (const row of dates) {
    ...
    if (dayText === new Date().toISOString().slice(0, 10)) {
      ...
      holdingsValue = Number(live.value);
    }
    ...
    await db.query(
      ...
      [account.player_id, row.d, cash, holdingsValue, total],
    );
    console.log(
      `[portfolio-values] ${account.player_id} ${new Date(row.d).toISOString().slice(0, 10)} -> $${(total / 100).toFixed(2)}`,
    );
  }
}
await db.end();
