...
export default async function DemoPage({
  ...
  if (!path.length)
    ...
    content = path[1] ? (
      <CategoryPreview category={path[1]} base="/demo" />
    ) : (
      <TradeOptions base="/demo" />
    );
    ...
    content = player ? (
      <>
        <PortfolioView
          ...
          own={false}
        />
        <Link className="text-link" href="/demo/leaderboard">
          ← Back to the crew
        </Link>
      </>
    ) : (
      <>
        <PageIntro
          eyebrow="Pine Ridge Explorers"
          title="Crew leaderboard"
          text="Cheer on your friends and explore their collections."
        />
        <LeaderList
          players={demoPlayers.map((p) => ({
            ...p,
            total_value_cents:
              p.cash_cents +
              p.holdings.reduce((n, h) => n + h.current_value_cents, 0),
          }))}
          ownId="jamie"
          base="/demo"
        />
