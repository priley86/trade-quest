...
import { getSportsProduct } from "../../../../lib/cardsight";
...
export default async function SportsDetail({
  ...
  const p = await getSportsProduct(id, filters?.sport || "");
  if (!p) notFound();
  return (
    <AppShell active="trade" profile={profile}>
      ...
      <section className="pokemon-detail">
        ...
        <div>
          ...
          <h1>{p.name}</h1>
          <p className="detail-price">{money(p.marketPriceCents)}</p>
          <p>Market price used in TradeQuest</p>
          <SportsBuy product={p} />
        ...
        <div className="price-history">
          ...
          <dl className="snapshot-grid">
            <div>
              <dt>Market price</dt>
              <dd>{money(p.marketPriceCents)}</dd>
            </div>
            <div>
              <dt>Low sale price</dt>
              <dd>{money(p.lowCents)}</dd>
            </div>
            <div>
              <dt>Mid sale price</dt>
              <dd>{money(p.midCents)}</dd>
            </div>
            <div>
              <dt>High sale price</dt>
              <dd>{money(p.highCents)}</dd>
            </div>
          </dl>
          {p.recentRecords?.length ? (
            <>
              <h2 className="recent-sales-heading">10 most recent sales</h2>
              <table>
                <thead>
                  <tr>
                    <th>Title</th>
                ...
                <tbody>
                  {p.recentRecords.map((r, i) => (
                    <tr key={r.date + "-" + i}>
                      <td>{r.title}</td>
                      <td>{money(Math.round(r.price * 100))}</td>
