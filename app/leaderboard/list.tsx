...
export function LeaderList({
  ...
  const ranked = [...players].sort(
    (a, b) =>
      (b.total_value_cents ?? -1) - (a.total_value_cents ?? -1) ||
    ...
    <section className="leaderboard">
      ...
      {!ranked.length && (
        <p className="empty-copy">
          Your crew’s first explorer will appear here soon.
        </p>
      )}
      {ranked.map((p, index) => (
        <Link
          href={`${base}/leaderboard/${p.player_id}`}
          className={`ranking-row ${p.player_id === ownId ? "you" : ""}`}
          key={p.player_id}
        >
          <span className="rank">
          ...
          <strong>
            {p.total_value_cents === null
              ? "Getting ready"
              : money(p.total_value_cents)}
          </strong>
