...
export function TradeOptions({ base = "" }: { base?: string }) {
  return <><PageIntro eyebrow="The Trading Post" title="What do you want to explore?" text="Pick a world below. Buying and selling will open in a future quest." />
    <section className="trade-grid">{options.map(o => <Link href={`${base}/trade/${o.type}`} key={o.type} className={`trade-option ${o.color}`}><span className="mini-badge">Coming soon</span><CategoryIcon type={o.type} large /><h2>{o.title}</h2><p>{o.copy}</p><span className="explore">Explore {o.title} <b>→</b></span></Link>)}</section>
