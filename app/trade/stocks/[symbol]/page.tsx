...
import { getStock } from "../../../../lib/alpaca";
      ...
      <section className="pokemon-detail">
        ...
        <div className="price-history">
          ...
          <dl className="snapshot-grid">
            ...
            <div>
              <dt>Daily change</dt>
              <dd className={stock.change >= 0 ? "positive" : "negative"}>
                {stock.change >= 0 ? "+" : ""}${stock.change.toFixed(2)}
              </dd>
            </div>
            <div>
              <dt>1-month change</dt>
              <dd className={pct(month) >= 0 ? "positive" : "negative"}>
                {pct(month) >= 0 ? "+" : ""}
                {pct(month).toFixed(1)}%
              </dd>
            </div>
            <div>
              <dt>1-year change</dt>
              <dd className={pct(year) >= 0 ? "positive" : "negative"}>
                {pct(year) >= 0 ? "+" : ""}
                {pct(year).toFixed(1)}%
              </dd>
            </div>
